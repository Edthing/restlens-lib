 ...
 */
export function findViolationLine(
  key: ViolationKey,
  content: string,
  message?: string
): LinePosition {
 ...
 */
export function flattenViolationsWithLines(
 ...
 */
export function buildViolationSummary(violations: FlatViolation[]): {
  totalViolations: number;
  errorCount: number;
  warningCount: number;
  infoCount: number;
  byRule: Array<{ ruleId: string; ruleName: string; severity: Severity; count: number }>;
} {
  const byRule = new Map<string, { ruleId: string; ruleName: string; severity: Severity; count: number }>();
 ...
 */
function findInfoLine(content: string, lines: string[]): LinePosition | null {
